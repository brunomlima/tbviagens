...
public partial class Admin_AdminPacotesImagens1 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            lblcaminhodaimagem.Text = pc.CaminhoImagem1.ToString();
            if (pc.CaminhoImagem.ToString().Trim() != "") { btnExcluiImagem.Visible = true; } else { btnExcluiImagem.Visible = false; };
            ImgPacote.ImageUrl = "~\\PACOTE\\" + pc.Codigo + "\\" + pc.CaminhoImagem1.ToString();
        }
    }
    ...
    protected void btnEnviar_Click(object sender, EventArgs e)
    {
        ...
        if (!ValidaTamanhodaImagem(FileUploadImagem.PostedFile.InputStream, 1140, 400))
        {
            lblMensagem.Visible = true;
            lblMensagem.Text = "Tamanho da imagem fora do padrão - Utilize uma imagem 1140px x 400px ";
            return;
        }
        if (lblTitulo.Text != "")
        {
            ...
            if (pcExcuir.CaminhoImagem1.ToString().Trim() != "")
                if (System.IO.File.Exists(Request.ServerVariables["APPL_PHYSICAL_PATH"] + @"PACOTE\" + pcExcuir.Codigo + "\\" + pcExcuir.CaminhoImagem1))
                {
                   System.IO.File.Delete(Request.ServerVariables["APPL_PHYSICAL_PATH"] + @"PACOTE\" + pcExcuir.Codigo + "\\" + pcExcuir.CaminhoImagem1);
                }
            pcExcuir.CaminhoImagem = "";
            pcExcuir.AtualizarImagem1();
        }
        Pacote pc = new Pacote();
        pc.Carregar(int.Parse(Request.QueryString["CD_PACOTE"].ToString()));
        pc.CaminhoImagem1 = FileUploadImagem.FileName;
        pc.AtualizarImagem1();
        ...
        Response.Redirect("AdminPacotesImagens1.aspx?cd_pacote=" + pc.Codigo);
    }
    ...
    protected void btnExcluiImagem_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (pc.CaminhoImagem1.ToString().Trim() != "")
            if (System.IO.File.Exists(Request.ServerVariables["APPL_PHYSICAL_PATH"] + @"PACOTE\" + pc.Codigo + "\\" + pc.CaminhoImagem1))
            {
                System.IO.File.Delete(Request.ServerVariables["APPL_PHYSICAL_PATH"] + @"PACOTE\" + pc.Codigo + "\\" + pc.CaminhoImagem1);
            }
        pc.CaminhoImagem1 = "";
        pc.AtualizarImagem1();
