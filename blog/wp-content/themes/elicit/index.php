	...
	<div class="content-area col-sm-12 col-md-8">
		...
		<div class="blog-post">
			...
			<div class="metadata">
				...
				<?php endif; ?><?php if (!is_page() && !is_single()): ?><span class="thecomment"><i class="fa fa-comments"></i><?php comments_popup_link(__(' Nenhum Comentário', 'elicit'), __(' 1 Comentários', 'elicit'), __(' % Comentários', 'elicit'), '', __(' Comentários Fechado', 'elicit') ); ?></span>
			</div>
			...
			the_excerpt();
			echo '<a href="' . get_permalink() . '"><br><button class="Button" type="button">Leia mais </button></a>';
			} else {
			the_content( '' );
			} ?>
			<?php endif; ?>
		...
		$prev_link = get_previous_posts_link(__('&laquo; Posts mais recentes', 'elicit'));
		$next_link = get_next_posts_link(__('Posts antigos &raquo;', 'elicit'));
		?>
